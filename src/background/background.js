...
const dispatchKeyEvent = (tabId, type, key, code, vkCode) => {
...
const simulateSlideNavKey = async (tabId, key) => {
...
let handleNextSlide = async () => {
    ...
    let tab = ((await chrome.tabs.query({ active: true, currentWindow: true }))[0]) || null;
